<template> 
  <c-form-card title="参数管理">
    <template slot="card-content">
      ...
      <v-flex xs12 sm12 md12 lg12 xl12>
        <v-form ref="form" class="elevation-16 pa-4" color="primary">
            <v-row align="center">
              ...
              <v-col cols="12"  xs="12" md="6" lg="4">
                <v-select     v-model="condtions.paramType" :items="ptype" label="是否系统参数"  dark clearable transition clear-icon="mdi-close-circle-outline"/>
              </v-col>
            </v-row>
        ...
        <v-data-table
         ...
         <!-- 
          ...
          <template v-slot:item.action="{ item }">
            ...
            <v-btn v-if="item.paramType !='1'"  tile small text dark height="32" width="66" class="mr-0 body-2" @click="removeItem(item)"><v-icon left size="16" class="mr-1">delete</v-icon>删除</v-btn>
          </template>
        </v-data-table>
        <!--  -->
      ...
      <v-flex xs12 sm12 md12 lg12 xl12>
        <c-dialog :title="title" :width="600" :height="440" :dialog="dialog" @save="save" @close="close">
               ...
               <v-form class="pa-2" color="primary" dark>
                  <v-row>
                    <v-col cols="12"  xs="12" md="12" lg="12">
                      ...
                      <v-text-field :readonly="ro" label="参数名称" v-model="fields.paramName" @input="$v.fields.paramName.$touch()"  @blur="$v.fields.paramName.$touch()" :error-messages="nameErrors" :counter="20" required hint="参数名称"/>
                    </v-col>
                    <v-col cols="12"  xs="12" md="12" lg="12">
                      <v-text-field :readonly="ro" label="参数键名" v-model="fields.paramKey"  @input="$v.fields.paramKey.$touch()"  @blur="$v.fields.paramKey.$touch()" :error-messages="keyErrors" :counter="40" required hint="多个参数使用统一的键名进行管理" />
                    </v-col>
                    <v-col cols="12"  xs="12" md="12" lg="12">
                    ...
                    <v-col cols="12"  xs="12" md="12" lg="12" style="display:none">
 ...
 export default {
      ...
      data: () => ({
          headers: [ //headers表头数组，里面是对象 //left，center，rights三种对其方式
            {text: '参数名称',   align: 'start',  value: 'paramName',   sortable: true},
            {text: '参数键名',   align: 'start',  value: 'paramKey' ,   sortable: true },
            {text: '参数键值',   align: 'start',  value: 'paramVal' ,   sortable: true },
            {text: '系统参数',   align: 'center', value: 'paramType' ,  sortable: false },
            {text: '描述',       align: 'start',  value: 'remark' ,  sortable: false  , width: 200},
      ...
      methods: {
        ...
        editItem (item) {
          ...
          this.title="修改参数";
          this.opration="edit";
          this.ro = this.fields.paramType === '0' ? false : true ;
          this.dialog = true  ;
        },
        removeItem (item) {
