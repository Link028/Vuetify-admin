<template> <!-- no-click-animation  -->
  <v-dialog dark persistent :id="cid" scrollable v-model="open" :dialog="dialog" :fullscreen="fullscreen" :width="width" class="primary elevation-16" style="z-index:99999999 !important;"><!-- style="z-index:1033 !important;" -->
    <v-card class="primary" dark>
      ...
      <v-card-text :style="heightsty" class="pl-4 pr-4">
        <v-container>
          <v-layout row wrap>
      ...
      <v-toolbar flat dense color="transparent" height="48" class="primary darken-0">
        <v-spacer class="white--text"></v-spacer>
        <v-btn tile text dark outlined height="36" width="80" class="mr-4" @click.stop="save"><v-icon left size="17">fa-save</v-icon> 保存</v-btn>
        <v-btn tile text dark outlined height="36" width="80" class="mr-4" @click.stop="close"><v-icon left size="17">fa-close</v-icon> 取消</v-btn>
      </v-toolbar>
    </v-card>
  </v-dialog>
</template>
...
export default {
  ...
  data() {
    return { 
      ...
      fullscreen : this.fc ,
      fl : this.fc ,
      heightsty: "height:"+this.height+"px;"
    }
  },
  methods: {
